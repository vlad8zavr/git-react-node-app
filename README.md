...
Код приложения переписан на **typescript**

## Содержание

1. [Установка](#установка)
2. [Запуск (для typescript)](#запуск)
...
### `ts-node server.ts <path to repo>`

(Например, `ts-node server.ts ./`)

(
    Если вдруг сервер не запускается, можно попробовать установить `ts-node` глобально
